...
owner = "bogdanfinn"
repo = "tls-client"
url = f"https://api.github.com/repos/{owner}/{repo}/releases/188652237"
...
async def get_latest_release() -> Tuple[str, list]:
    ...
    if response.status_code == 200:
        ...
        version_num = data["tag_name"].replace(
            "v", ""
...
async def download_and_save_asset(
    asset_url: str, asset_name: str, version: str
) -> None:
    # Download
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        if response.status_code != 200:
            raise TLSClientException(
                f"Failed to download asset {asset_name}. Status code: {response.status_code}"
            )
...
async def download_if_necessary():
    ...
    if len(download_url) == 0:
        raise TLSClientException(
            f"Unable to find asset {asset_name} for version {version_num}."
        )
...
async def update_if_necessary():
    ...
    if not current_asset or not current_version:
        raise TLSClientException(
            "Unable to read version info, no TLS libs found, use download_if_necessary()"
        )
    ...
    if version_num != current_version:
        print(
            f">> Current version {current_version} is outdated, downloading the latest TLS release..."
        )
